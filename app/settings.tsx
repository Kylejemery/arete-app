...
import { ensureRemindersScheduled, scheduleReminders } from '@/lib/reminders';
...
export default function SettingsScreen() {
  ...
  const requestPermissions = async () => {
    ...
    const asking = before.status !== 'granted';
    if (asking) logEvent('push_prompt_shown', { surface: 'settings' });
    const { status } = await Notifications.requestPermissionsAsync();
    if (status !== 'granted') {
      if (asking) logEvent('push_permission_denied', { surface: 'settings' });
      Alert.alert(
    ...
    if (asking) {
      logEvent('push_permission_granted', { surface: 'settings' });
      // Newly granted: the defaults that read "on" below are now really on.
      ensureRemindersScheduled('settings').catch(() => {});
    }
  };
  ...
  const scheduleNotifications = async (settings: any, silent = false): Promise<boolean> => {
    // The scheduler lives in lib/reminders.ts, shared with the dispatch nudge.
