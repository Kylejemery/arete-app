// Mirror of web/src/lib/checkinMessage.ts. The morning and evening check-ins
// are sent to the Cabinet as a synthetic user message ("[Morning check-in]
// Kyle has just completed ...") and stored in the thread so the counselors
// keep context. That text was never meant to be read back as if the user
// typed it (retention plan R4): the thread renders it as a compact chip.
// Rows written before this change carry no `kind`, so the prefix is the
// fallback signal.
