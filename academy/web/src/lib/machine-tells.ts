...
The rule under all of them: show it, do not say it. Never tell the reader an idea is important, surprising, or subtle. Write it so that it is.`
...
export const MACHINE_TELLS_SUMMARY =
  'a dash used between clauses or to fence off an aside (flag EVERY one); the negation-first frame ("This is not X. It is Y.", "not just X, but Y"); a line announcing an idea\'s importance instead of showing it ("the difference matters more than almost anything", "here is the thing", "this is the crux", "that changes everything"); over-the-top comparison or hyperbole ("nothing less than", "the single most"); the rule-of-three reflex; signposting (firstly, in conclusion, it is important to note); hedge stacks; thesaurus diction (delve, tapestry, underscore); uniform sentence rhythm; a rhetorical question that answers itself; a colon reveal or one-word sentence used as a punchline; an ending that summarizes'
