...
Rules:
- FIRST, judge whether the passage is argued prose at all. If it is reference apparatus — a table of contents, index, bibliography, list of abbreviations, copyright or title page, or acknowledgements — reply with exactly NON-PROSE and nothing else. No explanation, no partial summary. Summarizing an index produces a chunk that reads like scholarship but only reports which words appear where, and it competes at retrieval time with the real treatment of those same terms. Judge the content, not the length: a book's back matter can run many pages and still be nothing but apparatus.
