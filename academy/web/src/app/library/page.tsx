...
// A ledger entry: the same dream with its FULL text (the teaser truncates
...
function Observatory({ go, onDebate }: { go: (r: Room) => void; onDebate: (concept: string) => void }) {
  ...
  // from the sidebar) or the whole ledger.
  const [ledger, setLedger] = useState<FullDream[] | null>(null);
  const [ledgerNote, setLedgerNote] = useState<string | null>(null);
  const [dreamView, setDreamView] = useState<{ mode: 'one'; id: string } | { mode: 'all' } | null>(null);
  const openDreams = useCallback(async (view: { mode: 'one'; id: string } | { mode: 'all' }) => {
    setDreamView(view);
    if (ledger) return;
    try {
      const res = await fetch('/api/observatory/dreams?all=1');
      const json = await res.json();
      if (res.ok && Array.isArray(json.dreams)) setLedger(json.dreams);
      else setLedgerNote('The ledger could not be opened just now.');
    } catch {
      setLedgerNote('The ledger could not be opened just now.');
    }
  }, [ledger]);
  useEffect(() => {
    if (!dreamView) return;
    const onKey = (e: KeyboardEvent) => { if (e.key === 'Escape') setDreamView(null); };
  ...
  useEffect(() => {
    ...
    // The Dreaming Agent's approved conjecture — "The Corpus Imagines"; a
    // failed fetch just leaves the section empty.
    (async () => {
      try {
        const res = await fetch('/api/observatory/dreams');
        const json = await res.json();
  ...
  return (
    <main className="lib-obs-room" style={{ height: '100%', display: 'flex', minHeight: 0 }}>
      ...
      {/* dream overlay: one thought, or the whole ledger — full text either
          way. The framing line is load-bearing: these are thoughts FROM the
          corpus, reviewed by a human, never the words of any historical
          thinker. */}
      {dreamView && (
        <div onClick={() => setDreamView(null)} style={{ position: 'fixed', inset: 0, zIndex: 60, background: 'rgba(4,8,18,0.7)', backdropFilter: 'blur(3px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div onClick={e => e.stopPropagation()} className="lib-fade" style={{ maxWidth: 640, width: '100%', maxHeight: '85vh', overflowY: 'auto', background: 'linear-gradient(170deg,rgba(18,27,54,0.97),rgba(10,16,34,0.97))', border: '1px solid rgba(154,122,217,0.4)', borderRadius: 16, padding: '26px 28px', boxShadow: '0 18px 60px rgba(0,0,0,0.6)' }}>
            <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 12, marginBottom: 10 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#9a7ad9', boxShadow: '0 0 8px 1px rgba(154,122,217,0.7)' }} />
                <span style={{ fontFamily: MONO, fontSize: 9, letterSpacing: '0.22em', textTransform: 'uppercase', color: GOLD }}>
            ...
            {!ledger && !ledgerNote && (
              <p style={{ fontFamily: SERIF, fontStyle: 'italic', fontSize: 16, color: MUTED, margin: 0 }}>Opening the ledger…</p>
            )}
            {ledgerNote && (
              <p style={{ fontFamily: SERIF, fontStyle: 'italic', fontSize: 16, color: MUTED, margin: 0 }}>{ledgerNote}</p>
            )}
            {ledger && (dreamView.mode === 'one' ? ledger.filter(d => d.id === dreamView.id) : ledger).map(d => (
              <div key={d.id} style={{ marginBottom: 26 }}>
                <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 8, marginBottom: 8 }}>
                  <span style={{ fontFamily: MONO, fontSize: 8.5, letterSpacing: '0.18em', textTransform: 'uppercase', color: '#9a7ad9' }}>{DREAM_TYPE_LABEL[d.dreamType] || d.dreamType}</span>
                  {d.starred && <span style={{ fontFamily: MONO, fontSize: 8.5, letterSpacing: '0.14em', textTransform: 'uppercase', color: GOLD_L }}>✶ starred in review</span>}
                  {d.dreamWeek && <span style={{ fontFamily: MONO, fontSize: 8.5, letterSpacing: '0.1em', color: MUTED }}>{d.dreamWeek}</span>}
                </div>
                {d.title && <div style={{ fontFamily: SERIF, fontSize: 21, lineHeight: 1.2, color: IVORY, marginBottom: 8 }}>{d.title}</div>}
                {d.content && <p style={{ fontFamily: SERIF, fontStyle: 'italic', fontSize: 17.5, lineHeight: 1.62, color: IVORY, margin: '0 0 10px', whiteSpace: 'pre-wrap' }}>{d.content}</p>}
                {d.seedSummary && (
                  <p style={{ fontFamily: SERIF, fontSize: 13.5, lineHeight: 1.5, color: MUTED, margin: '0 0 6px' }}>{d.seedSummary}</p>
                )}
                {d.seedAuthors.length > 0 && (
                  <div style={{ fontFamily: MONO, fontSize: 9, letterSpacing: '0.1em', textTransform: 'uppercase', color: MUTED }}>
                    Dreamed from: {d.seedAuthors.join(' · ')}
