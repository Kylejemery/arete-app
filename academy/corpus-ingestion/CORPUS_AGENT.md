...
| Column | Meaning |
| --- | --- |
| `chunk_strategy` | A `chunker.js` strategy name. `paragraph` (default) is the 400-word window with no locator. `headed` cuts a work at its own BOOK (or ESSAY), optional PART, and CHAPTER / SECT. / LIFE OF / SONG headings, or short all-caps title lines when a book has no headed sections: every row gets a locator `book.section` ("14.9" for City of God XIV.9, "1.3.14" for Treatise 1.3.14) and a section label carrying the heading, and long sections are cut at paragraph boundaries into ~400-word rows that share the locator. `numbered` makes one row per numbered paragraph with the number as locator ("9" for Beyond Good and Evil §9, "1.13" for Genealogy I.13). Nothing before the first book or section heading is kept, so a translator's preface and a contents list never enter. The per-work strategies (`meditations-long`, `discourses`, `enchiridion`, `seneca-letters`) are also accepted. |
