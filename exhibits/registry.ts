// The native exhibit registry.
//
// An exhibit whose kind is 'native' is drawn by a component in this app
// rather than loaded from the web. Its row carries a component_key, and
// this file is the only place a key becomes a component. Adding a native
// exhibit is adding one line to `registry` below.
//
// The web app keeps its own registry at web/src/exhibits/registry.ts, at
// the same path relative to its source root and with the same shape. It
// has to: React Native components and DOM components are not
// interchangeable, and the two apps are separate TypeScript projects with
// no shared package. A key that is native on both platforms appears in
// both files; a key present in neither is a row the template will refuse
// to render, loudly, rather than showing a blank frame.
//
