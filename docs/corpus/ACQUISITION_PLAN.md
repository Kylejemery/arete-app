...
4. **Every new work is registered against the question map** at ingest, with at least one position and role. A work that bears on no question in the map either belongs to a question the map is missing, in which case add the question, or does not belong in the corpus. Since 2026-09-14 the map is data: `corpus_questions` holds the fifteen questions and `corpus_question_registrations` holds one row per (question, work) with position and role. The Paper Agent proposes registrations with each summary, the admin review card edits them, and approval writes them; de-ingest removes them. Other ingest paths register by hand until they carry the same field.
