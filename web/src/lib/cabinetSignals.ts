import { getItem, setItem, getTodayDateKey } from './storage';
import type { Goal, JournalEntry } from './types';

// Cabinet signals — app-internal data the counselors can see, monitor, and
// comment on. These come from the user's own activity inside Arete: focus
// sessions and accountability meta-signals (journaling gaps, stale goals).
// No new permissions, no external data, so they are injected for every tier.
...
export async function getPomodoroHistory(): Promise<PomodoroHistory> {
  try {
    const raw = getItem(POMODORO_SESSIONS_KEY);
    if (!raw) return {};
...
export async function buildFocusContext(): Promise<string> {
  ...
  try {
    const history = await getPomodoroHistory();
    const todayCount = history[getTodayDateKey()] ?? 0;
    lines.push(`- Today: ${todayCount === 0 ? 'none completed yet' : `${todayCount} completed`}.`);

    const cutoff = new Date();
    cutoff.setDate(cutoff.getDate() - 6);
    const weekKeys = Object.keys(history).filter((k) => k >= getTodayDateKey(cutoff));
